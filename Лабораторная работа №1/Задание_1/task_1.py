...
class LocalTrain:
    """
    Класс описывает пригородный электропоезд(электричка)
    у данного класса три атрибута, а именно маршрут,
    количество вагонов и скорость в данный момент.
    Пример использования:
    >>> trein1 = LocalTrain('Финляндский-НевскаяДубровка', 8, 60)
    ...
    def __init__(self, itinerary: str,
                 number_of_wagons: int, speed: (float, int)):
        """
        ...
        if not isinstance(number_of_wagons, int) or number_of_wagons <= 0:
            raise ValueError(
                "Количество вагонов должно быть целым положительным числом")
    ...
    def direction(self, num: int):
        ...
        # Проверка аргумента
        if not isinstance(
                num, int) or num <= 0:  # Проверка переданного значения
            raise ValueError(
                'в качестве параметра необходимо передать целое положительное число')
    ...
    def change_speed(self, change: (float, int)):
        ...
        if change < 0:
            print(
                f'Скорость уменьшена на {abs(change)} км/ч и теперь составляет {self.speed} км/ч')
        else:
            print(
                f'Скорость увеличена на {change} км/ч и теперь составляет {self.speed} км/ч')
        return self.speed
...
class Triangle:
    ...
    def __init__(self, name: str, side1: (int, float),
                 side2: (int, float), side3: (int, float)):
        """
        ...
        if not isinstance(name, str) or len(
                name) != 3 or len(set(name)) != len(name):
            raise TypeError(
                "Название треугольника должно быть строкой из трех различных символов")
        if not isinstance(side1, (int, float)) or side1 <= 0:
            raise ValueError(
                "Длина стороны 1 должна быть положительным числом")
        if not isinstance(side2, (int, float)) or side2 <= 0:
            raise ValueError(
                "Длина стороны 2 должна быть положительным числом")
        if not isinstance(side3, (int, float)) or side3 <= 0:
            raise ValueError(
                "Длина стороны 3 должна быть положительным числом")
        # Проверка существования треугольника
        arrside = [side1, side2, side3]
        n = arrside.index(max(arrside))
        if sum(arrside) - arrside[n] <= arrside[n]:
            raise ValueError(
    ...
    def square(self):
        ...
        p = (self.side1 + self.side2 + self.side3) / \
            2  # Определение полупериметра
        S = (p * (p - self.side1) * (p - self.side2) *
             (p - self.side3)) ** 0.5  # Нахождение площади по формуле
        # Вывод сообщения оо проведенной операции
    ...
    def find_angle(self, name_of_angle: str):
        ...
        if not isinstance(name_of_angle, str) or len(
                name_of_angle) != 1 or name_of_angle not in self.name:
            raise ValueError("Название угла должно состоять из одного символа, "
                             "который должен входить в название треугольника")
        # Подсчет размера угла в соответсвии с его расположением при помощи
        # теоремы косинусов
        ...
        print(
            f'Угол {name_of_angle} треугольника '
            f'{self.name} = {angel}')  # Выведение сообщения
        return angel
...
class House:
    ...
    'Private'
    >>> house1.coverage()
    Покрытие изменено на краска, теперь материал дома: Дерево покрытие краска
    """
    ...
    def __init__(self, material: str, floors: int):
        ...
        if not isinstance(material, str):
            raise TypeError(
                "Материал из которого построен дом должен быть строкой")
        if not isinstance(floors, int) or floors <= 0:
            raise ValueError(
                "Количество этажей должно быть целым положительным числом")
        # Присуждение значений параметров в характеристики объекта класса
    ...
    def change_floor(self, num: int, action: str):
        ...
        if not isinstance(action, str) or action not in ['+', '-']:
            raise ValueError(
                "Действие должно быть строкой и соответсвовать двум варантам: + или -")
        # Выполнения действия с этажами
        if action == '+':
            self.floors += num
            print(
                f'Количество этажей увеличенно на {num}, и теперь составляет '
                f'{self.floors}')
        else:
            if num > self.floors:  # Проверка того что необходимо снести меньше этажей чем было изначально
                raise ValueError(
                    "Количество этажей для сноса не должно превышать изначальное количество")
            self.floors -= num
            print(
                f'Количество этажей уменьшенно на {num}, и теперь составляет '
                f'{self.floors}')
    ...
    def decide_type(self):
        ...
        return typ
    # Метод со значением аргумента по умолчанию
    def coverage(self, typ='краска'):
        """
        Метод добавляет к материалу дома покрытие либо меняет имеющееся и выводит сообщение об изменениях
        :param typ: отвечает за тип покрытия, по умолчанию краска, должен быть типа str
        :return: None, ничего не возвращает так как изменяет характеристику класса
        """
        if not isinstance(typ, str):    # Проверка переданного параметра
            raise TypeError
        # Превращаю строку в массив
        arr_material = self.material.split()
        # Если покрытие уже добавленно, то изменяю его тип, если нет, добавляют информацию о покрытии
        # в характеристику материала
        if len(arr_material) == 1:
            new_material = self.material + ' покрытие' + ' ' + typ
            self.material = new_material
        else:
            self.material = arr_material[0] + ' ' + arr_material[1] + ' ' + typ
        # Вывожу сообщение о выполнении проделанной операции
        print(f"Покрытие изменено на {typ}, теперь материал дома: {self.material}")
